...
jQuery.fn.motionCaptcha || (function($) {
	...
	$.fn.motionCaptcha = function(options) {
		...
		return this.each(function() {
			...
			// Set random shape
			var shpRandResult = Math.floor(Math.random() * ((opts.shapes.length)-1));
			$canvas.addClass( opts.shapes[shpRandResult] );
	...
	$.fn.motionCaptcha.defaults = {
		...
		onSuccess: function($form, $canvas, ctx) { 
			//custom by sahrul start
			$.getJSON(host+'pub/getRegURI/'+$form[0].id, function(resp) {
				$(opts.actionId).val(resp.uri);
	        });
			//custom by sahrul end
