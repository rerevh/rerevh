	...
	const fv   = FormValidation.formValidation(form,       			
        			 {
        				fields: {
        					...
        					jobgroup          : { validators: { notEmpty    : { message: 'Required.' },
        						                              stringLength: { min:3,max:100,message: 'Name min 3 character'},
                                                  regexp      : { regexp: /^[A-Za-z\/\s\.\,\']+$/,message: 'Name not contains number or symbol'} }},
        					grade        : { validators: { notEmpty    : { message: 'Required.' }}},	
        					title             : { validators: { notEmpty    : { message: 'Required.' },
        						                              stringLength: { min:3,max:100,message: 'Name min 3 character'},
                                                  regexp      : { regexp: /^[A-Za-z\/\s\.\,\']+$/,message: 'Name not contains number or symbol'} }}, 
                  skill_name    : { validators: { notEmpty    : { enabled: false, message: 'Required.' }}},	
                  skill_code    : { validators: { notEmpty    : { enabled: false, message: 'Required.' }}},	  
                  subskill_name : { validators: { notEmpty    : { enabled: false, message: 'Required.' }}},	
                  subskill_code : { validators: { notEmpty    : { enabled: false, message: 'Required.' }}},	   
                  skill_id_select : { validators: { notEmpty    : { enabled: false, message: 'Required.' }}},	
                  subskill_id_select : { validators: { notEmpty    : { enabled: false, message: 'Required.' }}},	            
                  
                  job_name        : { validators: { notEmpty    : { message: 'Required.' }}},	
                  job_grade       : { validators: { notEmpty    : { message: 'Required.' }}},	
                  job_jf          : { validators: { notEmpty    : { message: 'Required.' }}},	
                  job_sjf         : { validators: { notEmpty    : { message: 'Required.' }}},	
                  klasifikasi         : { validators: { notEmpty    : { message: 'Required.' }}},	
                  type         : { validators: { notEmpty    : { message: 'Required.' }}},	
                  orgrisk         : { validators: { notEmpty    : { message: 'Required.' }}},	
                ...
                let enabled = false;
                form.querySelector('[name="job_families"]').addEventListener('input', function (e) {
                    const job_families = e.target.value;
                    alert(job_families);
