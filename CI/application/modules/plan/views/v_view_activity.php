	...
	<header class="page-header">
		...
		<div class="right-wrapper pull-right">
			<ol class="breadcrumbs">
				...
				<li><span>Monthly Activity</span></li>
            </ol>
		...
		<section class="panel">
			<header class="panel-heading" style=" border: 1px solid #5B707B; padding: 10px 14px; background: #5B707B;">
				<h2 class="panel-title" style="font-size: medium; color: white;"><i class="fa fa-chevron-right"> </i> Monthly Activity</h2>
			</header>
			   <div>
                                                                      ...
                                                                      <div class="modal-content">
                                                                        ...
                                                                        <div class="modal-body">
                                                                                    <p class="message"></p>
                                                                                     ...
                                                                                     <input type="hidden" class="form-control" id="kode_kegiatan_update" name="kode_kegiatan_update" value="">
...
$(document).ready(function() 
{
    ...
    $(document).on('click','.cancel',function(ee){
      ...
      $(document).on('change','#status_update',function(ee){
            ...
            var val = $(this).val();
            var id = $("#id_status_update").val();
            var kode_kegiatan=$("#kode_kegiatan_update").val();
            if(val=="2")
            {
                reason(id);
            }
            else if(val=="3")
            {
            ...
            else if(val=="1")
             {
                            $.ajax({
                               ...
                               url : BASE_URL+"index.php/Plan/Monthly_activity/set_session_kodekegiatan"
                       })
             }
      })
      ...
      $(document).on('click','#update_status',function(ee){
             ...
             var kode_kegiatan=$(this).data("kodekegiatan");
             $("#id_status_update").val(id);
             $("#kode_kegiatan_update").val(kode_kegiatan);
             
             $('.form_status_update').html("");
             //$("#status_update option[value=]").attr("selected","selected");
             $('#status_update').val($(this).find('option:first').val());
             $("#status_update option[text=" + status +"]").attr("selected","selected");
     ...
     $(document).on('click','#btn_save',function(ee){
             ...
             $.ajax({
                 url: BASE_URL+"index.php/Plan/Monthly_activity/simpan",
                 data: formData,
      ...
      $(document).on('click','#btn_save_status_update',function(ee){
            ...
            var month = $("#month").val();
            var kode_kegiatan = $("#kode_kegiatan_update").val();
            save_status_update(id_status_update,status_update,reason,month,kode_kegiatan);
      })
})
function tabel()
{
        $.ajax({
                    type: "POST",
                    url : BASE_URL+"index.php/Plan/Monthly_activity/tabel",
...
function form()
{
        $.ajax({
                    type: "POST",
                    url : BASE_URL+"index.php/Plan/Monthly_activity/form",
                    beforeSend : function(jqXHR,setting)
                    {
...
function reason(id)
{
        $.ajax({
                    type: "POST",
                    url : BASE_URL+"index.php/Plan/Monthly_activity/alasan",
                    data :
                            {
                                id:id
                            },
...
function postponse(id)
{
        $.ajax({
                    type: "POST",
                    url : BASE_URL+"index.php/Plan/Monthly_activity/postponse",
                    data :
                            {
                                id:id
                            },
...
function save_status_update(id_status_update,status_update,reason,month,kode_kegiatan)
{
    $.ajax({
         url: BASE_URL+"index.php/Plan/Monthly_activity/save_status_update",
         data:
                 {
         ...
         success: function(databack) {
            ...
            if(databack=='s') {
                ...
                if(status_update=="1")
                {
                        window.location.href = BASE_URL+'index.php/Request/DirectFund';
                }
            } else {
...
function save()
{
    ...
    $.ajax({
         url: BASE_URL+"index.php/Plan/Monthly_activity/save",
...
function proses_edit()
{
    ...
    $.ajax({
         url: BASE_URL+"index.php/Plan/Monthly_activity/proses_edit",
         data: $('#submit').serialize(),
         type: "POST",
         dataType : "json",
         beforeSend: function() {
...
{
    $.ajax({
               url : BASE_URL+"index.php/Plan/Monthly_activity/delete",
               data: 
                       {
                           id:id
                       },
